package net.ghoul.practice.arena.selection;
...
public class ArenaSelectionListener implements Listener {
    ...
    public void onPlayerInteractEvent(PlayerInteractEvent event) {
        ...
        if (item != null && item.equals(Selection.SELECTION_WAND)) {
            ...
            String message = CC.GREEN + (location == 1 ? "First" : "Second") +
                    " location " + CC.DARK_GRAY + "(" + CC.GRAY +
                    clicked.getX() + CC.DARK_GRAY + ", " + CC.GRAY +
                    clicked.getY() + CC.DARK_GRAY + ", " + CC.GRAY +
                    clicked.getZ() + CC.DARK_GRAY + ")" + CC.GREEN + " has been set!";

            if (selection.isFullObject()) {
                message += CC.DARK_GRAY + " (" + CC.GRAY + selection.getCuboid().volume() + CC.GREEN + " blocks" +
                        CC.DARK_GRAY + ")";
            }
