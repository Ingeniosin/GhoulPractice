...
import net.ghoul.practice.kiteditor.KitEditRules;
...
public class Kit {
    ...
    @Setter @Getter private int inventorySlot = 69;
    @Setter @Getter private boolean glow = false;
    @Setter @Getter private boolean customDamages = true;
    ...
    public void delete() {
        ...
        Ghoul.getInstance().getKitsConfig().getConfiguration().set("kits." + getName(), null);
        try {
            Ghoul.getInstance().getKitsConfig().getConfiguration().save(Ghoul.getInstance().getKitsConfig().getFile());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static void preload() {
        FileConfiguration config = Ghoul.getInstance().getKitsConfig().getConfiguration();
        for (String key : config.getConfigurationSection("kits").getKeys(false)) {
            ...
            kit.setInventorySlot(config.getInt(path + ".inventorySlot"));

            kit.setGlow(config.getBoolean(path + ".glow"));

            kit.setCustomDamages(config.getBoolean(path + ".customDamages"));
    ...
    public void save() {
        ...
        BasicConfigurationFile configFile = Ghoul.getInstance().getKitsConfig();
        ...
        configFile.getConfiguration().set(path + ".inventorySlot", inventorySlot);
        configFile.getConfiguration().set(path + ".glow", glow);
        configFile.getConfiguration().set(path + ".customDamages", customDamages);
    ...
    public void updateKitLeaderboards() {
        ...
        for (Document document : Profile.getAllProfiles().find().sort(Sorts.descending("kitStatistics." + getName() + ".elo")).limit(15).into(new ArrayList<>())) {
